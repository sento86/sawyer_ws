...
#JUST change this to 1 for non-filtering torque/force data
#use around 10 to get a nice signal and remove noise
FILTER_WINDOW_SIZE = 20

class Sensor_filtered():
    """
    a simple class to filter data
    """
    ...
    def append(self, val0, val1, val2, val3, val4):
        self.sensor0.append(val0)
        self.sensor1.append(val1)
        self.sensor2.append(val2)
        self.sensor3.append(val3)
        self.sensor4.append(val4)

        if len(self.sensor0) > self.window:
            self.sensor0.pop(0)
            self.sensor1.pop(0)
            self.sensor2.pop(0)
            self.sensor3.pop(0)
    ...
    def get_mean_values(self):
        m0 = sum(self.sensor0)/len(self.sensor0)
        m1 = sum(self.sensor1)/len(self.sensor1)
        m2 = sum(self.sensor2)/len(self.sensor2)
        m3 = sum(self.sensor3)/len(self.sensor3)
        m4 = sum(self.sensor4)/len(self.sensor4)
        #print 'values:', m0, m1, m2, m3, m4
        #print 'lengths:', len(self.sensor0), len(self.sensor1)
...
class SMC_Class(object):
	...
	# maintain pressure and perpendicularity
        #self._time = 0.0
        #self._step = 0.0
        #self._theta = 0.0
        #self._r = 0.0
...
#        self._sub_netft = rospy.Subscriber("/tool_adapt_link/ft_sensor/ft_sensor_topic", WrenchStamped, self._ig)
        self._sub_netft = rospy.Subscriber("/sawyer_ft_sensor/adapt_l/ft_sensor_topic", WrenchStamped, self._ig)
        print "Suscribed to force/torque sensor"
    ...
    def Control_loop_Cart(self):
        ...
        while not rospy.is_shutdown()  and not self._done:
            ...
            # self._Sensor[3] = msg.wrench.torque.x*1.0
            # self._Sensor[4] = -msg.wrench.torque.y*1.0

            self.sns_filt.append(-msg.wrench.force.y,
                            msg.wrench.force.x,
                            -msg.wrench.force.z,
                            msg.wrench.torque.x,
                            msg.wrench.torque.y)

            val0, val1, val2, val3, val4 = self.sns_filt.get_mean_values()

            self._Sensor[0] = val0
            self._Sensor[1] = val1
            self._Sensor[2] = val2
            self._Sensor[3] = 10.0*val3
            self._Sensor[4] = 10.0*val4
            ...
            if self._nContact:
#                self._p_ref[2] = self._p_ref[2] -0.05/self._rate
                self._p_ref[2] = self._p_ref[2] -0.05/self._rate
